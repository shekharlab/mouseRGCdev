XgboostAssign = function(train_object, test_object, var.genes = NULL, id_train="typeID", test.label=NULL, do.scale=TRUE, newID = "xgboost", plot.validation = FALSE, max.cells.per.ident=1000, min.cells.per.ident = 200){
  
  train_object = set.all.ident(train_object, "typeID")
  
  # XGBoost Step
  bst_atlas= XGBoost_train(train_object, var.genes = var.genes, do.scale=do.scale, plot.validation=FALSE,max.cells.per.ident=1000, min.cells.per.ident = 200)
  thresh.use=0.7
  test_Data <- as.matrix(test_object@data[var.genes,])
  ...
  transfer.label_bst = apply(test_pred_bst,2,function(x){ 
    if (which.max(x) <= 40){
      if (max(x) > 0.5){ which.max(x) } else {46}
    } else {
      if (max(x) > 0.4){ which.max(x)} else {46}
    }
    
  })
  names(transfer.label_bst) = rownames(test_object@data.info)
  test_object@data.info[,newID] = factor(transfer.label_bst[rownames(test_object@data.info)])
  
  # Graph voting
  test_object@ident = newID
  return(test_object)
  
}

XGBoost_train = function(train_object, var.genes = NULL, do.scale=FALSE, max.cells.per.ident = 1000, min.cells.per.ident = 200, train.frac = 0.9, weights = TRUE, plot.validation=FALSE){
  
  # Subsetting and Scaling
  train_Data = as.matrix(train_object@data[var.genes,])
  ...
  for (cc in c(1:length(levels(train_object@ident)))){
    i = levels(train_object@ident)[cc]
    cells.in.clust =which.cells(train_object,i);
    ...
    if (n < min.cells.per.ident){
      train.temp = sample(train.temp, min.cells.per.ident, replace = TRUE)
    }
    
    validation.temp = setdiff(cells.in.clust, unique(train.temp))
  ...
  if (plot.validation){
    ...
    colnames(A) = levels(train_object@ident); rownames(A) = colnames(A)
...
HarmonizeXgboostRGC = function(test_object, ID1 = "xgboost01", ID2 = "xgboost02", newID="xgboost"){
  ...
  label1 = test_object@data.info[,ID1]; names(label1) = cell.names
  label2 = test_object@data.info[,ID2]; names(label2) = cell.names
  ...
  test_object@data.info[,newID] = new_label
  test_object@ident = newID
  return(test_object)
}
...
GraphAssignment = function(object, k=60, knn=15, ref.cells = NULL, target.cells = NULL, null.ident = 46, reduction.use = "tsne"){
  ...
  if (reduction.use == "tsne" | reduction.use == "UMAP") k = 2
  ...
  while (l==0){
    ...
    if (is.null(ref.cells)){
      ref.cells = setdiff(rownames(object@dr[[reduction.use]]@cell.embeddings), cells.unassigned)
    }
    
    if (is.null(target.cells)){ 
      frac.unassigned = length(cells.unassigned)/(length(rownames(object@dr[[reduction.use]]@cell.embeddings)))
    ...
    Xquery = object@dr[[reduction.use]]@cell.embeddings[cells.to.assign,1:k]
    X = object@dr[[reduction.use]]@cell.embeddings[ref.cells,1:k]
